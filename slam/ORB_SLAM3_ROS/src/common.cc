...
void Common::SetupRosPublishers(ros::NodeHandle &node_handler)
{
    ...
    this->odomPub = node_handler.advertise<nav_msgs::Odometry>("orb_slam3/odom", 10);
...
void Common::PublishPositionAsOdometry(Sophus::SE3f position, ros::Time current_frame_time, bool to_target) {
    tf2::Transform tf_position = TransformFromMat(position);
    tf2::Transform tf_position_target;
    tf2::Stamped<tf2::Transform> tf_position_target_stamped;
    ...
    {
        tf_position_target = TransformToTarget(tf_position, this->camFrameId, this->targetFrameId);
        tf_position_target_stamped = tf2::Stamped<tf2::Transform>(tf_position_target, current_frame_time, this->worldFrameId);
    }
    else
    {
        tf_position_target_stamped = tf2::Stamped<tf2::Transform>(tf_position, current_frame_time, this->worldFrameId);
    ...
    odom_msg.header.frame_id = this->worldFrameId;  // 通常是 "map" 或 "odom"
    odom_msg.child_frame_id = this->camFrameId;     // 通常是相机坐标系
