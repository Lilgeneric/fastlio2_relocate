    ...
    if (rayShape) {
        ...
        switch (publishPointCloudType) {
            ...
            case livox_laser_simulation_CUSTOM_MSG:
                ...
                break;
            case LIVOX_ROS_DRIVER2_CUSTOM_MSG:
                PublishLivoxROSDriver2CustomMsg(points_pair);
                break;
            default:
                break;
        }
...
void LivoxPointsPlugin::PublishLivoxROSDriver2CustomMsg(std::vector<std::pair<int, AviaRotateInfo>> &points_pair) {
    // 1. 获取雷达参数
    auto rayCount = RayCount();
    auto verticalRayCount = VerticalRayCount();
    auto angle_min = AngleMin().Radian();
    ...
    // 3. 创建Livox ROS Driver2自定义消息
    livox_ros_driver2::CustomMsg msg;
    msg.header.frame_id = frameName;  // 必须与算法配置的坐标系一致
    msg.header.stamp = ros::Time::now();
    
    // 4. 设置时间基准（关键修正：使用uint64_t类型）
    struct timespec tn;
    clock_gettime(CLOCK_REALTIME, &tn);
    msg.timebase = static_cast<uint64_t>(tn.tv_sec * 1e9 + tn.tv_nsec);  // 纳秒级时间基准
    msg.lidar_id = 0;  // 默认设备ID
    msg.rsvd = {0, 0, 0};  // 显式初始化保留字段

    // 5. 处理点云数据
    for (int i = 0; i < points_pair.size(); ++i) {
        auto &pair = points_pair[i];
        int verticle_index = roundf((pair.second.zenith - verticle_min) / verticle_incre);
        int horizon_index = roundf((pair.second.azimuth - angle_min) / angle_incre);
        ...
        if (verticle_index < verticalRayCount && horizon_index < rayCount) {
            auto range = rayShape->GetRange(pair.first);
            auto intensity = rayShape->GetRetro(pair.first);
            if (range >= maxDist || range <= minDist || abs(range) <= 1e-5) range = 0.0;

            // 6. 计算3D坐标
            ignition::math::Quaterniond ray;
            ray.Euler(ignition::math::Vector3d(0.0, pair.second.zenith, pair.second.azimuth));
            auto axis = ray * ignition::math::Vector3d(1.0, 0.0, 0.0);
            auto point = range * axis;

            // 7. 填充Livox点数据（关键修正）
            livox_ros_driver2::CustomPoint pt;
            pt.x = static_cast<float>(point.X());  // 必须转为float
            pt.y = static_cast<float>(point.Y());
            pt.z = static_cast<float>(point.Z());
            pt.reflectivity = static_cast<uint8_t>(intensity * 255.0);  // 反射率0-255
            pt.tag = (intensity < 0.1) ? 0x04 : 0x00;  // 动态标记噪点
            pt.line = static_cast<uint8_t>(pair.second.line);  // 线号必须为uint8_t
            pt.offset_time = static_cast<uint32_t>(i * 5);  // 200kHz采样率下每点间隔5μs
